...
class ServerThread extends Thread {
    Socket server;
    boolean flag = true; //thread 구동 여부를 결정하는 변수
    ...
    public void run() {
        ...
        while (flag) {
            ...
            try {
                ...
                //여기서 문제가 발생했다는 건
//                지금 스레드가 담당하고 있는 client가 종료되었다라고 추측할 수 있다
//                따라서 현재 스레드는 더 이상 구동시키지 않고 종료해줄 필요가 있다.
                System.out.print(this.getName() + " 스레드에서 문제 발생: ");
                System.out.println("클라이언트에서 보내온 메시지 수신 중 문제 발생");
                flag = false; //while 문이 더이상 동작하지 않게 됨 -> thread 종료 효과
            }
...
//			수신된 메시지를 모든 클라이언트로 송신
            ...
            for (int i = 0; i < Server.totalSocket.size(); i++) {
                ...
                try {
                    ...
                    //client랑 연결되어 있던 종이컵이 아직 저장공간에 남아있어서
                    //해당 종이컵을 사용해서 메세지를 보내려고 하면 문제가 발생
                    //->해결 방법은 이제 더 이상 사용하지 않는 종이컵을 제거
                    Server.totalSocket.remove(i);
                    System.out.print(this.getName() + " 스레드에서 문제 발생: ");
                    System.out.println("모든 클라이언트에 메시지 전송 중 문제 발생");
                }
            }
        }
    }
